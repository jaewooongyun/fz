...
**키잉 규칙 (교훈 서술 방식)**: 교훈은 도구·맥락 한정이 아닌 **실패 클래스**로 서술 — 같은 실패 클래스의 다른 발현에서 recall되도록. 예: "Codex 출력 head 금지" ✕ → "전수 주장 근거 수집 시 출력 잘림 금지" ○. (위 표의 도구/CLI 버전·설정 기록 항목과 무관 — 본 규칙은 교훈 제목·서술 방식만 대상)
