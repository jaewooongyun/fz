...
5. ⛔ SPM Chore 검증 (새 패키지 생성 시 필수):
  ...
  8. ⛔ 형제 샘플(convention) 수집 — 4번 소비자 전수 수집과 같은 Grep 패스에서 함께 한다:
     ...
     - ⛔ **관례와 같다는 것이 면제는 아니다** — 형제의 그 형태를 정당화한 근거가 이 대상에도 성립하는지 함께 묻는다.
       근거가 형제 자신의 속성이면 형태만 옮긴 것은 답습이다. 판정 기계는 `modules/review-structural-axes.md` §E 가 정본이며
       여기서 다시 정의하지 않는다 (같은 규칙을 두 곳에 적으면 한쪽만 고쳐진다). 항목 7 의 `redundant_import` 는 이 질문을 **import 입도**로 구체화한 것이다.
