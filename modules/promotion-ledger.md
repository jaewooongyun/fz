...
- 내용: `modules/cross-validation.md` § Coverage Gate 절차 1이 *"대상 **파일** 전수 목록"* 이라 단위를 파일로 고정한다. 주장이 파일 안의 항목을 수량화하면 파일 1/1 = 100% 로 통과한다
- generalize: broad (문서·코드 무관) | 과적합 위험: 中 — ⛔ **'항목 단위' 일괄 강제는 과구조화**다(§5 원칙 3). 주장이 수량화하는 명사와 같은 단위여야 한다
- 근거: [verified: 회귀 fixture `tests/fixtures/cross-validation/coverage-units/run.sh` 케이스 S — 파일 100% 대 항목 14%]
- ⛔ 활성 차단: **세션 3건 충족**(F-022·F-045·F-098 별개) + **회귀 fixture 1개 충족**(위 러너 S·F 케이스). 잔여 = **외부 채점 1회**
- ⚠️ negative 케이스 병기 — 러너의 `F file-claim 2/2` 는 발화하지 **않아야** 하는 케이스다(파일 단위 주장에는 파일 분모가 정답). 과잉 교정 방어이며 F-050(works) 과 같은 축
- 승격 목표 (트랙 D): 외부 채점 1회 후 `cross-validation.md` 절차 1 문구 교체 판정. ⛔ 새 Gate 행 추가 없음 — 기존 문장 교체
...
- 승격 목표 (트랙 D): 별개 세션 1건 추가 + 외부 채점 1회 후 절차 6 처방 교체("재실행" → "다른 조달원 또는 Σ부분=전체")
