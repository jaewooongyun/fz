...
### L-15: 반복 집합의 형태 판단 — **양방향** (F-031 ↔ F-286)

⛔ **한쪽만 고치면 시계추가 반대로 간다.** 두 관측이 같은 뿌리의 반대 극이다.

| 방향 | 관측 | 실패 | 사용자 판정 |
|---|---|---|---|
| 위양성 | `F-031` (TVG-5085) | 처방이 *"닮았으면 합쳐라"* 한쪽으로 기울어 있다 | `near-identical` 만으로는 **합칠 이유가 안 된다** |
| 위음성 | `F-286` (TVG-8072) | 형제와 맞췄다는 것만으로 **안 합칠 이유**로 삼았다 | *"무조건 형제라고 추가하는 게 아니라 구조를 더 좋은 쪽으로"* |

**공통 뿌리 = 판단 축 부재.** 둘 다 *국소 일관성*을 기본값으로 두고 **집합 전체의 최적 형태**를 묻지 않는다.

- 관측 #1: `F-031` | Date: 2026-08-24 | finding-source: internal(사용자 catch + CodeRabbit Major)
  - 두 파일이 차이 2줄까지 수렴했는데 보고하지 않음. ⛔ **결함 A** — `fz-code` 마찰 신호는
    이 케이스를 정확히 서술했으나 `[candidate]` 이고 **원장 항목이 없어** 관측 #2 를 쌓을 자리가 없었다
- 관측 #2: `F-286` | Date: 2026-09-21 | finding-source: internal(PR #4920 리뷰어 1코멘트)
  - 기존 방어 3종 전부 미발동 — 분기 폭증(대상 밖) · 코드 반복(**처방은 맞는데 트리거가 함수 단위라 안 닿음**) ·
    peer slot 비대칭(**구조적으로 반대 방향** — 완벽히 대칭이라 침묵)
  - ⛔ `CLAUDE.md` 「기존 스타일 매칭」·「drive-by 금지」가 **오히려 나쁜 구조의 근거로 작동**했다
  - 파이프라인 5지점(plan 대안·writeScope·S3 게이트·GPT verify·code 마찰) 전부 침묵

⚡ 조치 (2026-09-21): `fz-code/SKILL.md` 마찰 표에 **「통합 후보 — 내가 더한 항이 내가 도입한 술어와 등가」**
candidate 신설. `peer slot 비대칭` **바로 뒤**에 두어 두 방향이 한 자리에서 읽히게 했다.
발동 조건은 좁다 — *"같은 변경에서 내가 도입한"* + *"논리곱/합이 같다"*(기계 판정).
⛔ 이것이 `F-031` 이 기각한 *"닮았으면 합쳐라"* 로 번지지 않는 이유다.
보조: `주석-추상화 불일치` 에 **축2**(주석이 단일 술어 선언인데 코드가 N분할) 추가.
부수: `modules/gates.md` 에 게이트 작성 self-check(*"고정하는 형태보다 단순한 등가가 있는가"*) 신설.

- 관측 #3: 조치의 **배선 범위 갭** | Date: 2026-09-22 | finding-source: internal(사용자 catch — *"형제를 무조건 따라가는 문제"*)
  - 관측 #2 의 ⚡조치는 `fz-code` 마찰 표와 `modules/gates.md` 에만 닿았다. **review·peer-review 경로는 그대로였다** —
    실측: 그쪽 형제 규칙은 한 방향뿐이었다(`review-checks.md` *"3+ 형제 동일 → severity 하향"* ·
    `review-structural-axes.md` §E *"형제 3+ 같은 형태면 관례, 지적 금지"*). 반대 방향은 `import` 입도에만 있었다.
  - ⛔ **카운트 미반영** — 관측 #2 와 **같은 세션**이다(`같은 세션 다발은 1 count` — L-5 선례). evidence 는 2 sessions 유지.
  - ⚡ 조치: `review-structural-axes.md` §E 에 양방향 쌍을 **주 배선**으로 넣었다(항상 켜지는 경로).
    `review-checks.md` 항목 8 은 정본을 지목하는 **보조**로만 둔다 — 같은 규칙을 두 곳에 적으면 한쪽만 고쳐진다.
    근거 축 태깅 기계는 `L-12` 를 재사용한다(신규 정의 없음).
  - ⭐ 일반화: **조치를 한 스킬에만 배선하면 다른 스킬 경로는 계속 침묵한다.** `F-031` 결함 A(원장 항목 부재)의
    한 층 위 버전이다 — 항목은 있었고 *배선 범위*가 좁았다. 다음 조치부터 "이 규칙이 지나가야 할 경로 전부인가"를 묻는다.

- 승격 판정: **evidence 2 sessions** (트랙 A 임계 5 미달 — 계속 관측). 다음 관측은 여기 append 한다.
- ⛔ `F-031` 결함 A 가 이 항목 부재였다 — **이 블록이 그 수리다.** candidate 를 만들 때 원장 항목을
  같이 만들지 않으면 그 규칙은 영원히 활성화되지 않는다(L-2 가 올바른 선례).

### L-16: 같은 사실의 선언처 전수 (F-287 ↔ F-289)

같은 사실이 **여러 자리에 적히는** 구조에서, 검사와 수정은 한 자리만 본다.

| 관측 | 형태 |
|---|---|
| `F-287` | 인용 논문이 레지스트리 **둘 다에** 없었다 — 어느 쪽도 자기가 전부라고 보지 않았다 |
| `F-289` | 인벤토리를 문서 **둘이** 선언하는데 검사기는 하나만 봤다 (안 보는 쪽 6개 중 5개 어긋남) |

- 관측 #1: `F-287`·`F-289` | Date: 2026-09-21 | finding-source: internal(self — 감사 중 발견)
  - 같은 세션에서 재현 1건 추가: VerifySpec 이 워크플로 **4곳**에 선언돼 있었고, 한 곳만 고치면
    살아 있는 정본이 여전히 필드를 거부했다(`additionalProperties: false`). 같은 세션이라 **1 count**.
- 오라클 (기계 판정 가능 — 이 점이 `L-5` 와 갈리는 지점):
  ```
  선언처 집합 == 변경·검사 대상 집합 ?
  검사 항목 집합 == 선언 항목 합집합 ?
  ```
  ⛔ 둘 다 묻는다. `F-289` 는 첫째만 고쳤을 때 둘째가 남았다.
- generalize: broad (선언 중복은 문서·스키마·인벤토리에 편재) | 과적합 위험: 低 (셀 수 있는 오라클)
- ⚠️ **`L-5` 와 별개**: 상위 축(함께 움직여야 하는 집합의 전원 커버)은 공유하나 **해소 방식이 상이**하다 —
  `L-5` 는 상보 쌍이라 설계 지식이 필요해 *질문 형태* 로만 가능하고, 본 항목은 **선언처를 세는 grep** 으로
  기계 판정된다. `L-12` 가 `L-7` 에 쓴 판단 선례를 따른다. cross-link: [[L-5]]
- ⛔ 활성 차단: evidence 1 session → candidate. 활성 전환은 트랙 A **5 sessions** 후.
- 승격 목표 (Track A): 5 sessions + GPT verify → `fz-review` 조건부 검증 또는 `lint_contracts` 검사 신설 판정.
...
- ⛔ **2026-09-01 D-2 외부 채점이 본 항의 보류를 독립 확인**했다 [외부: codex]: *"promotion-ledger.md(당시 :370-372) 는 별개 세션 2건에 도달해야 한다고 규정하지만 현재 1건이라고 기록한다. 외부 채점만으로 이 내부 승격 조건을 충족할 수 없으므로 교체 2는 아직 보류 대상이다."* → D-2 를 적용하면서 본 항 문구는 **건드리지 않았다**
  - ⛔ **인용 속 줄번호는 채점 당시 좌표다** — 원문이므로 고치지 않는다(외부 판정 재포장 금지).
    ...
    (`guides/harness-engineering.md` § 문서 규약 · `guides/agent-team-guide.md` § Same-model Cross-Verify).
...
### OBS-02 잔여 항목 처분 (2026-09-01)

30건 감사의 잔여분이다. ⛔ **조사를 더 하지 않는다는 기록이지 결함이 사라졌다는 기록이 아니다.**
각 항목의 진입 조건을 적어 두고, 조건이 오면 그때 착수한다.
...
| `review-live` 증거 경로 0 · `code-pair` 전용 arg 0 · `review-team` 트리 선언만 | 관찰 유지 | 해당 경로를 실제로 쓰는 세션에서 결손이 드러나면 |

⛔ **`판정 영역 구분자 부재` 는 반증 확정이라 목록에서 뺐다** — `negative-clean.md` 오라클은
코드펜스 안이고, 펜스 인식 스캐너로 재측정해 전제가 거짓임을 확인했다.

⛔ **`파이프라인 15/19 registry 미등재` 는 프레이밍이 뒤집혔다.** 19종 전부가
`modules/pipelines.md` 에 자기 트리거 행을 갖고 `/fz` Phase 3.1 이 그것을 읽는다(19/19 실측).
결함은 누락 15건이 아니라 registry 의 *"유일한 정본"* 선언 한 문장이었고 그것을 고쳤다.
⛔ 원 판정대로 15행을 옮겨 적었다면 트리거가 두 파일에 중복돼 서서히 어긋났을 것이다.
...
## P2 신규 항목 (A5 — 2026-09-22 사용자 승인)

> ⛔ **임계를 새로 정하지 않았다.** 위 P2→P1(1건) · P1→P0(2건) 을 그대로 쓴다. A5 가 한 일은
> **동일 수정 계약을 공유하는 집합에 P-ID 를 부여**한 것뿐이다.
> ⛔ **축 수 증가를 승격 근거로 쓰지 않는다** — 승격은 P-ID 단위로 ① 동일 수정 계약 ② 독립 session
> ③ 회귀 fixture ④ 외부 채점을 본다. 무관한 사례를 한 축에 두 건 넣으면 재발 없이 통과한다.
>
> 파편화 실측: 엔트리 **253** · `failure_class` 고유 **249** · 이름 2회 이상 **3종/7건**.
> ⇒ 이름 단위로는 어떤 임계에도 닿지 않아, 앞 3종은 이름으로 · 뒤 3종은 **처방 공유**로 묶었다.

| P-ID | 조치 이름 | 묶은 근거 | 엔트리 |
|---|---|---|---|
| `P2-D` | lint 가 통합 이름 드리프트를 잡는다 | 이름 동일 ×3 | `F-204`·`F-205`·`F-211` |
| `P2-E` | 확정 전 전이 지점을 읽는다 | 이름 동일 ×2 | `F-009`·`F-017` |
| `P2-F` | 에이전트 정의는 세션 시작에 고정된다 | 이름 동일 ×2 | `F-228`·`F-233` |
| `P2-G` | 같은 사실의 **선언처를 전수**로 본다 | 처방 공유 — 검사가 선언처 하나만 본다 | `F-287`·`F-289` |
| `P2-H` | 측정 도구의 **무응답을 대상의 정상으로 읽지 않는다** | 처방 공유 — 도구 부재·검사기 중단이 같은 오귀속 | `F-288`·`F-290` |
| `P2-I` | 주장과 측정의 **주어·술어를 대조**한다 | 처방 공유 — 주어 불일치(F-292)·술어 과매칭(F-293) | `F-292`·`F-293` |

**관측 카운트**: 전 항목 **관측 #1** (같은 세션에서 등재됐으므로 `같은 세션 다발은 1 count`).
⛔ P2→P1 은 **eligible session 1건 + `/fz-gpt verify` approved + 사용자 승인** 이며 아직 충족되지 않았다.

### 스키마 계약 (`p_id`)

| 축 | 확정 |
|---|---|
| 필드명 | `p_id` (snake_case — 기존 `failure_class`·`gap_type` 과 정합) |
| 타입·형식 | 문자열 `P{tier}-{letter}` — ⚠️ **실측 정정**: 제안서는 `P{tier}-{slug}` 였으나 실제 규약은 **letter** 다(`P1-B`·`P2-A`) |
| 필수 여부 | **선택** — 묶이는 항목만 갖는다. 전건 필수로 하면 249개 1:1 P-ID 가 생겨 파편화가 필드만 바꿔 재현된다 |
| 누락값 | **미분류**로 읽는다. ⛔ 부재 = *승격 자격 없음* 이 아니다 |
| 스키마 버전 | 필드 미신설 — 소비자가 미지 필드를 무시하므로 전진 호환된다. 계약은 이 절이 갖는다 |
...
> 미결이 세 벌로 흩어져 있었고, 어느 쪽을 고쳐야 하는지가 문서에 없었다.

| 축 | 상태 (2026-09-18 실측) |
|---|---|
| 실행 경로 | **Workflow 전면 전환 완료** (Wave 1~4) — `workflows/*.js` 8개가 fan-out·수렴을 소유 |
