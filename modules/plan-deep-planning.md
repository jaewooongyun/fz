...
- [⛔ Phase 1 Gate는 SKILL.md에 보존 (트리밍 비저하 원칙 — single source: `guides/prompt-optimization.md` §1 보충 3a)](#-phase-1-gate는-skillmd에-보존-트리밍-비저하-원칙--single-source-guidesprompt-optimizationmd-1-보충-3a)
- [절차](#절차)
- [설계 원칙](#설계-원칙)
...
| Q5 접근 경계 | "차단/제거/캡슐화"를 의도한 접근 경로가 실제로 차단되는가? access modifier(public/internal/private)가 의도와 일치하는가? 기존 코드가 이벤트 채널을 우회하여 직접 호출하는 경로가 남아있지 않은가? **모듈화 작업 시 추가**: 모듈에 추가하는 각 public 타입에 대해 — (a) 타입의 필드/메서드가 모듈 책임 범위에 속하는가? (b) 도메인 특화 필드(비즈니스명, 하드코딩 UI 문자열)가 있는가? (c) 모듈 내부에서 실제 사용하는가, 아니면 pass-through인가? 하나라도 경계 밖이면 모듈에서 제외. **(d)** *[candidate: 1 session evidence]* 순수 프레젠테이션 + **공유 UI/디자인시스템 모듈**(`CLAUDE.md ## Shared Modules`)만 의존하는 신규 UI 타입이면 공유모듈 승격 후보인가 + 다른 모듈에 SwiftUI/UIKit 트윈이 기존재하는가(대칭 체크)? | 접근 제어 변경을 계획에 포함 + Anti-Pattern Constraints에 금지 패턴 기록. 모듈화 시: Concern Classification 테이블 작성 |
...
- **대안 패턴 / 구조 결정 옵션 테이블**: Q4에서 제시된 대안 + 현재 선택의 근거 + 핵심 구조 결정별 채택안·대안·1줄 trade-off (답습/미러링 작업이면 3축 필수, 핵심 구조 결정 없으면 N/A 1줄). PROCEED 판정이어도 plan에 포함하고 **사용자 보고 시 표로 제시** — 이유: 옵션은 기록이 아니라 제시가 목적 (ASD-1802 불만 ①)
