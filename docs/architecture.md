...
├── workflows/       8개 — 네이티브 Workflow 결정적 스크립트 (discover-adversarial, plan-collaborative, code-pair ...)
├── modules/         45개 — gates(완료 게이트 SSOT), team-core, pipelines, cross-validation, lead-action-default, gpt-strategy, memory-guide, fz-gpt-bash-hygiene, fz-gpt-subcommands-core/aux ...
│   (patterns/ 5개는 2026-09-21 `docs/history/patterns/` 로 이동 — 사료, 실행 절차 아님)
...
├── scripts/         34개 — gate_check(완료 판정기) · gate_stop_hook(종료 차단) · lint 3 · gpt-exec · health-check ...
