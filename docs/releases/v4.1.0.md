...
OBS-02 회귀 분석에서 도출된 **"함수 이름 ≠ 함수 책임 + 호출 중단 ≠ 정의 제거"** 패턴을 fz 생태계로 반영. `plan-impact` 에이전트의 Exhaustive Impact Scan을 `a~f` → `a~g`로 확장하고, `review-correctness` 에이전트에 Function Responsibility Audit를 추가.
...
D2(OBS-01) fix 에서 `extractBody` 헬퍼의 **호출을 중단**하면서, 그 함수 body 안의 `header.status 3xx-5xx` 검사 책임이 Serializer 층으로 이전되지 않음. 18+ LegacyResponse 소비자가 세션 만료(401) / 서버 에러(500)를 silent success로 처리하는 회귀 발생.
...
**근거**: harness-engineering §7 "단일 ablation 결과로 규칙 만들지 않는다" + 18차 규칙 1 "complexity 2배+ 분할 트리거" 준수. 1건 회귀(OBS-02) 기반 과적합 방지.
...
분석/계획/검증 문서 (외부 TVING/OBS-02 폴더):
...
- **근본 원인 발견**: Codex GPT-5.4 (D3 --deep review + OBS-02 1차 review 단독 발견)
