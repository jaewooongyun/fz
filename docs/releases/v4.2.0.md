...
1. **OBS-03 Scope Challenge**: `fz-plan` Phase 3에서 Codex verify 이슈를 **scope_disposition**으로 분류하여 "발견된 것 = 고쳐야 할 것" 자동 번역을 차단. Plan `§X/§Y/§Z` handoff 계약으로 **Read Scope(탐색)** vs **Write Scope(구현)** 분리.
...
## 🆕 Feature Set A: Scope Challenge (OBS-03)
...
- **내용**: 설계 문서에 사용자 홈 절대 경로 하드코딩 → 마켓플레이스 배포 시 이식성 훼손
...
- [OBS-03 Scope Challenge 원본 분석](https://github.com/jaewooongyun/fz) (TVING/monitoring/OBS-03)
