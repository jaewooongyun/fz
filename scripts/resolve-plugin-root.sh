...
#   `modules/cross-validation.md`의 `get_gpt_skill_path()`와 8개 호출부가 `FZ_PLUGIN_ROOT`를
#   **소비**하는데 레포 어디에도 **할당이 없었다**(실측: 소비 10곳 / 할당 0곳). 전부 빈 값이 전달돼
#   Tier 2b(플러그인 번들 `gpt-skills/`)가 **항상 건너뛰어졌다.**
...
#    `Base directory for this skill: …/skills/fz-gpt` 에서 `../..`를 유도해 **절대경로로** 1회 export한다.
...
# fail-closed 마커 검증 — `gpt-skills/`는 Tier 2b의 전제, `guides/`는 플러그인 식별자
if [ ! -d "$ROOT/gpt-skills" ] || [ ! -d "$ROOT/guides" ]; then
  echo "⛔ 플러그인 루트가 아님: $ROOT (gpt-skills/ 또는 guides/ 부재)" >&2
  exit 2
fi
