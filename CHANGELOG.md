...
않았고 그 변경은 사라졌다. 2026-09-10 에 base 신선도 결함이 **같은 증상으로 재관측**되고서야
유실이 드러났다 — `/fz-peer-review` 가 3커밋 뒤처진 base 로 원본 37개 중 16개를 틀리게 집었다.

⛔ **"반영됨" 의 정의가 없었다.** 작업 트리 적용을 반영으로 적었는데 작업 트리는 휘발성이고
표기는 영속이다. 유실 경위는 확정하지 못했다 — HEAD·stash·작업 트리 어디에도 없다는 사실까지다.

**복구(v4.31.1 유실분 4건)**: 텔레메트리 분모(읽지 못한 파일이 집계에서 사라짐) · 폴백 검증자
읽기 전용(`--sandbox` 는 CLI 성질이라 대체 경로로 상속 안 됨) · base 신선도(`ls-remote` 대조,
⛔ `fetch` 는 하지 않는다) · Agent 금지 스코프를 Gather 로 확장(5회차 재발).

**신규 수리 3건**: verify-gates 제목 verbatim(축약 → stale 거부 7/7) · `gpt-exec.sh` 경로를
`${FZ_PLUGIN_ROOT}` 기준으로(⛔ 헤딩은 앵커 때문에 미변경) · risk_scan `session` 저신뢰 분리.

⭐ risk_scan 에서 **실측이 계획을 정정했다** — 갈리는 축은 hunk 거리가 아니라 동반 토큰이
순증인가 개명인가였다. net-new auth 는 거리 무관 정당, 개명된 auth 는 같은 hunk 일 때만.
5케이스 fixture 고정.

**검증**: health-check 13검사 exit 0(매 커밋) · 회귀 runner 11→12 · risk-scan 케이스 11→16 ·
마커 866→868(전부 복구분, 신규 수리는 순증 0). 기능단위 7커밋.

⛔ **도구 문제**: `set -euo pipefail` 이 이 셸에서 발화하지 않는다(errexit on 보고, `false` 뒤
계속 실행). 시작 플래그 `bash -ec` 는 정상. 다단계 편집 실패가 뒤 명령의 성공 문구에 가려졌고
끝에 붙인 fail-closed assert 만 잡았다.
...
삽입하는데, 루프가 **시작 시점의 `Gate` 객체**를 계속 냈다. 도장이 하나 들어갈 때마다 뒤쪽
게이트의 스팬이 1씩 밀리므로 n번째 도장은 (n−1)줄 앞에 박힌다. 한동안은 자기 블록 안에 떨어져
**조용히 통과**하고, 어느 지점을 넘어서면 도장이 게이트 줄 앞으로 나가 확정본이 자기 파서에
거부된다 — 실행 게이트 7개가 넘는 원장이 exit 3 이었다. id 목록으로 돌면서 매 반복
**재로드본에서** 게이트를 다시 찾게 고쳤다.

⭐ 값이 더 큰 것은 두 번째 발견이다 — **`finalize` 경로를 실행하는 fixture 가 0건이었다.**
`tests/fixtures/gates/finalize/eight-gates.md` 를 신설했다. 수정을 되돌리면 게이트 self-test 가
67 에서 66 으로 떨어진다.

Tier 2 투표 계약은 문서 59곳이 세 갈래로 갈려 있었다(가 36 · 나 18 · 다 5). 값을 복사해 맞추지
않고 **정본(§ MergeContract § 9)을 지목**하는 방식으로 정합화했다 — 값 복사 0건이다. 전수
감사표와 grep 을 대조했다: `grep=59 audit=59 dup=0 missing=0 extra=0`. 값을 복사했다면 정본이
한 번 바뀔 때 스물세 곳이 조용히 어긋난다 — 이번에 고친 것이 정확히 그 상태다.

⭐ `scripts/check_g8_style.py` 의 줄표 게이트는 **매핑 블록 정본을 위반으로 세고 있었다.**
`modules/explanation-output.md` 가 담당 코드 줄을 `**담당 코드**: <경로> — <심볼>` 로 규정하는데,
그 구조 줄표가 산문 줄표와 같이 세어진다. 담당 코드 줄 **형태**에 한해 줄표 1개를 뺐다.
⛔ 라벨 줄 전부에서 빼는 판이 한 번 나왔고, 그러면 `**하는 일**: A — B` 같은 산문 줄표가 통째로
면제된다 — 판정 축은 라벨 이름이 아니라 값의 형태다. self-test 는 11/11 이다(신규 줄표 2건 포함).

Workflow `scriptPath` 거부에는 우회 계약이 없었다. ⛔ 이 실패는 `mode:'fallback'` 이 아니다 —
스크립트가 아예 실행되지 않아 반환에 `mode` 자체가 없고, 그것을 fallback 으로 읽으면 **SOLO
오강등**이 된다. 판별·복사·검증·재시도 4단계를 `guides/skill-authoring.md` 에 신설해 호출 지시
10곳과 에러표 3곳에서 지목했다. 그리고 `Workflow` 를 가진 소비자 SKILL.md 아홉 **전부에**
`Bash(grep *)`·`Bash(cp *)` 가 0이었다 — 권한이 없으면 우회는 정확히 그것이 필요한 상황에서
죽는다.

evidence 수집 대상은 작업 트리가 아니라 **ref** 다. `modules/evidence-collection.md` 의
`${BASE_REF}` 를 Gather 해석값으로 바꾸고(⛔ `upstream/` 접두를 고정하지 않는다), 빈 ref 를
fail-closed 로 막았다 — 빈 값은 작업 트리를 grep 하고 **exit 0** 을 낸다. zsh 는 unquoted glob 을
셸이 먼저 확장하고 매칭이 없으면 명령 실행 전에 중단하므로 따옴표를 강제했고, 산출 종료 전
`git status --porcelain` 을 요구한다.

⛔ **솔직하게 적는다.** 이 릴리즈의 수리가 **결함 7건을 새로 만들었고 교차검증이 그걸 잡았다** —
G8 줄표 위음성 · 권한 부재 · `/tmp` 경유 · ref 하드코딩 · `cp` 부모 디렉토리 · 산출 규약 2건.
그리고 마지막 두 Step 은 Codex spend cap 으로 **이종 검증 없이** 동종(fresh-context Claude)으로만
봤으며, 그 폴백 검증자가 `git checkout --` 로 미커밋 변경을 삭제했다(복구 완료 · 유실 0 확증).
read-only 보호가 Codex CLI 의 성질이라 대체 경로에 상속되지 않았다. 미결 F-121 과 보류 4 Step
은 릴리즈 노트 § 남은 것에 적었다.
