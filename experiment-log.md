...
| 8 | 2026-07-20 | 5(+스톨 재시도) | 0 | 3/3 | 0 | 초회 7287s **스톨 실패**(1 agent 6회 무진행 — 세션 3번째 스톨 패턴) → **resume 부분 재시도 성공**(사용자 교정: resume-first, 캐시 4 재생+1 재실행, 1193s) | ✅ 대상: plan-tvg2906-holes 플랜 문서 리뷰(비-diff, #3/#5 선례). findings 19(major 2·minor 17·FP 0): direction↔plan 카운트 disposition 상충(A1, Lead判 counter 채택 minor 정정)·H-P2→L-7 과병합 의심(조건부 격하)·순증 재산술(candidate 2→1)·브리프 전제 F-A5 반증(L-11 4회차 실효)·**감사↔플랜 직교 CONFIRMED**(file/line-disjoint). 6건 즉시 반영. Codex 불능 — 이종 상실 명시 |
...
### ⑥ 세션 effort sweep — plan Workflow (신설 2026-09-11)

> ⛔ **① 과 별개다.** ①(effort frontmatter)은 2026-06-14 **철회된 사료**이며 임계도 N=5·토큰 ≤30% 로 다르다 — 새 실험을 그 표에 쓰면 철회 기록을 덮는다 (GPT verify #2 지적).
> 배선 근거: `plan-collaborative.js` 11콜이 `effort: 'xhigh'` 고정. 공식 default 는 `high` 이고 *"run a fresh effort sweep"* 을 요구한다 [verified: platform.claude.com/docs/en/build-with-claude/effort]. ⛔ arm 은 **세션 레벨**로 설정한다 (`guides/skill-testing.md` §8.1 — per-call `opts.effort` 수정 금지, 실효는 `[미검증]`).
> 격리 요구 (GPT verify #3): 코드 상태(S1~S6 적용분) 고정 · 같은 requirement·codeContext · advisor 상태 동일 · control(xhigh)·treatment(high) **둘 다 fresh 실행**(과거 실행과 대조 금지 — 프롬프트·스키마가 이미 다르다) · run 별 세션 effort 실측 기록 · per-call 1콜 프로브 병행.
> 임계 (사전등록): **독립 채점자**(fresh-context) 판정 critical·major 손실 0 AND G2 5축 보존 → high 채택 근거. 시간·토큰은 같은 행에 함께 기록한다(품질만 보면 R1 을 못 잰다). **N=1 은 탐색**이며 확산은 N=3 누적 후.

| # | date | arm | session_effort | wall | critical_path | out_tok | thinking | advisor | G2 5축 | 채점자 판정(critical/major 손실) | 판정 |
|---|------|-----|----------------|------|---------------|---------|----------|---------|--------|------------------------------|------|
| 1 | 2026-09-13 | **control `xhigh`** (fresh) | `xhigh` — 트랜스크립트 `effort` 필드 실측(세션 11건·워커 3/3) | 1,079.9s | 564.1s | 151,100 | 86,436 | 0 | 요구 3축 closed | (기준) | `wf_64c31a67-14e` · 완주 오라클 agents 4/4 · journal_results 4/4 |
| 2 | 2026-09-13 | **treatment `high`** (fresh) | `high` — 동(세션 10건·워커 3/3) | **1,011.2s (-6.4%)** | 501.1s | 138,981 (-8.0%) | 72,866 (**-15.7%**) | 0 | 요구 3축 closed | blind `wf_623b7827-3b6` — **xhigh 고유 major 8 · high 고유 major 3** · verify 4:5 · overall **`Q-superior`**(Q=xhigh) | ⛔ **high 기각.** 사전등록 임계 '손실 0' 대비 **major 8건 손실**이고 §5.9 행 C 의 **노이즈 바닥 5를 넘는다** — 손실이 실행 분산으로 설명되지 않는 첫 사례. ⭐ 잃는 것이 한 종류다: **게이트가 무엇을 재는가**(public 증가분 수치·Lab sha256·임계값 불변·pre-fix 트리 강제·`safeAreaRegions=.container` 좌표 전제 실측). effort 를 내리면 플랜이 짧아지는 게 아니라 **검증이 헐거워진다**. 시간 이득 -6.4% 와 바꾸기에 작다. ⛔ N=1 |

## §5.9 구조 ablation — plan Workflow (신설 2026-09-11)

> ⛔ `§5.7` freeze 와 `§5.8` ①~⑤ 는 불가침 — 본 절은 **신설만** 이다(`§5.8` 신설 선례 계승).
> 측정 계약: `modules/peer-review-tiers.md` § 수집 축소의 검증 계약(ablation) 준용 — 같은 입력 2회(전량 control + 변형), 사전등록 기준, 사후 변경 금지.
> ⛔ **S8a(모델)와 S8b(구조)를 분리 평가한다** (GPT verify #4·#13): 한 번에 바꾸면 어느 것이 효과인지 갈리지 않는다. S8b 는 (i) S3 CC 제거와 (ii) S5→fresh-context refute 교체를 **다시 분리**한다.
> ⛔ "최종 생존 0" 은 무가치의 증거가 아니다 — 통합 단계 유실일 수 있다. must-fix **개수**가 같아도 고유 발견이 바뀔 수 있으므로 독립 채점자가 **동일성·정확성·축 커버리지**를 판정한다.

| # | date | 변형 | control | wall Δ | 고유 발견(control/변형) | 동일성 판정 | G2 커버리지 | null·병합 | 판정 |
|---|------|------|---------|--------|------------------------|------------|------------|-----------|------|
| 1 | 2026-09-11 | **단일 1콜**(구조 대조 S8-0) | 9-agent 6단계 | **3,621s → 805s (-78%)** | P(9-agent) 고유 8건(critical 0·major 5·minor 3) / Q(1콜) 고유 6건(**critical 1**·major 1·minor 4) | blind 채점 `wf_ac6e5638-9ff` — 요구 3축 **양쪽 closed** · command verify 실행가능 P4/Q7 | — | null 0 · fallback 0 | **P-superior** — 규칙상 전면 재설계 아님. ⭐**P 고유 8/8 이 edgeCases 인용**(impact·arch 렌즈 단독 기여 0) → 기여가 한 렌즈에 집중. ⛔**일반화 금지**: 벤치 트리에 구현이 이미 있어 감사형 과제로 기울었고(채점자 자인) Q 의 critical 은 '기전 신설 절차'였다 — pre-fix 트리 재측정 필요. N=1 |
| 2 | 2026-09-11 | **D1 lean 3콜**(생산∥edge적대→델타병합) | 9-agent 6단계 | **신규구현 3,492s→1,203s (-65.5%)** · 감사형 3,621s→1,191s (-67.1%) | pre-fix: 9콜 고유 10건(critical 0·**major 5**) / D1 고유 11건(critical 0·major 2) | blind 채점 2트리 — **신규구현 `wf_9e22d233-6f8` P-superior**(9콜 고유 0/5/5 · D1 0/2/9 · verify 3:3) / **감사형 `wf_92f69320-1e7` equivalent**(9콜 0/3/5 · D1 0/3/6 · verify **4:5**) · 요구 3축 4/4 셀 closed | — | null 0 · fallback 0 | ⛔ **D1 채택 불가** — 사전등록 임계 '검증된 critical·major 손실 0' 대비 **5건 손실**(wall 임계 ≤1,400s 는 충족). ⭐ 손실 출처: **impact 렌즈 2 · arch 렌즈 1 · integrate 3** — 감사형에서 '인용 0건'이던 impact·arch 가 신규구현에서는 major 3건을 낸다. **N=1 제거 판단이 뒤집혔다**(사용자가 지적한 벤치 편향이 실측으로 확인). 다음 수 = D1b(impact+arch 통합 콜을 **3번째 병렬 팔**로 추가 — 병렬이라 wall 불변 예상). ⭐ **감사형에서는 equivalent** 이고 D1 이 verify 실행가능에서 앞선다(4:5) — 9콜 산출의 command 4건이 **존재하지 않는 경로를 하드코딩**해 지정 트리에서 실행 불가였고(채점자 [verified: Glob]) D1 은 IMPL_ROOT 파라미터화 + 동결 트리 fail-closed 거부 프로토콜을 냈다. ⛔ 그래도 임계는 '손실 0' 이므로 채택 불가 — 골대를 옮기지 않는다 |
| 3 | 2026-09-12 | **D1b lean2 4콜**(생산∥edge∥impact+arch→델타병합) | 9-agent 6단계 (신규구현 트리) | **3,492s → 1,344s (-61.5%)** | 9콜 고유 8건(critical 0·**major 3**) / D1b 고유 10건(critical 0·**major 4**) | blind 채점 `wf_d192366e-7c3` — 요구 3축 양쪽 closed · command 실행가능 **4:5**(D1b 우세) | — | null 0 · fallback 0 | **overall `equivalent`** — D1 의 P-superior 에서 전환. 같은 트리 D1→D1b: 9콜 고유 major **5→3**, D1측 major **2→4**, verify **3:3→4:5**. ⛔ 그래도 사전등록 임계('손실 0')는 **미달**(3건) — 골대를 옮기지 않는다. 잔여 손실 출처 = **recheck(S5) 1 · arch 설계 1 · impact 심도 1** — 렌즈 부재가 아니라 **심도**다. ⭐ 다음 측정 = **노이즈 바닥 교정**(같은 구조 2회 실행의 상호 고유 발견) — 그것을 재기 전에는 '손실 3건' 이 구조 차이인지 실행 분산인지 갈리지 않는다 |
| **C** | 2026-09-12 | **동일 구조 재실행(노이즈 교정)** — 9콜 2회차 | 9콜 1회차 (같은 입력·스크립트·트리) | 3,492s vs **3,575s (2.4% 차)** | 1회차 고유 8건(critical 0·**major 1**) / 2회차 고유 12건(critical 0·**major 5**) | blind 채점 `wf_b1515a59-ab8` — 요구 3축 양쪽 closed · command 3:4 | — | null 0 · fallback 0 | ⛔⭐ **N_noise = 5** · overall **`Q-superior`**(equivalent 아님). **변인은 실행 난수 하나**인데 설계 기전까지 갈렸다 (1회차 = rect 레지스트리+GeometryReader / 2회차 = predicate API + **UIViewRepresentable 마커** — 1회차가 '선례 0건이라 보류'로 기각했던 안). ⇒ 사전등록 임계 **'손실 0' 은 도달 불가능한 값**이었다. 행 2·3 의 '손실 N' 은 구조 차이의 증거가 아니다 — D1b 손실 3 · D1 감사형 손실 3 은 **노이즈 바닥(5) 이하**, D1 신규구현 손실 5 는 **바닥과 동일**. ⛔ 이 교정도 **N=1** 이다 — '노이즈 바닥이 5' 가 아니라 '1회 관측에서 5' |
| **S8a** | 2026-09-13 | **렌즈 모델 sonnet**(edge·impact+arch 를 sonnet, 전체플랜·병합 opus 유지) | 동일 구조 전 opus (fresh, 런1) | 1,079.9s → 1,011.8s (-6.3%) | opus렌즈 고유 **critical 1**·major 3·minor 7 / sonnet렌즈 고유 major 1·minor 7 | blind `wf_6e5b3156-456` — 요구 3축 양쪽 closed · command 실행가능 **3:5**(opus 우세) · overall **`Q-superior`**(Q=opus) | — | null 0 · fallback 0 | ⛔ **sonnet 렌즈 기각.** 손실 4건은 노이즈 바닥 5 미만이나 **critical 은 성질이 다르다** — 행 C 에서 critical 은 양쪽 0건이었고 흔들린 것은 major 뿐이다. ⛔ 더 결정적인 것은 **사실 오류**: sonnet 렌즈 산출이 동결 pre-fix 트리 `prefix@938507b13` 을 *"Glob 0건·이미 구현됨"* 으로 단정해 `unresolved` 1번에 올렸다(채점자 [verified] 실측 반증 — 그 트리는 존재하고 관련 심볼 0건인 pre-fix). **렌즈가 입력 트리 상태를 거꾸로 읽었다.** ⭐ 시간 이득 -6.3% 는 구조적으로 나올 수 없다 — 렌즈는 임계 경로 밖이고 여유가 175~207s 다(런1 실측 전체플랜 794.5s vs 렌즈 619.7·587.7s). 실행 분산으로 본다. ⛔ N=1 |
| **채택** | 2026-09-12 | **D1b `plan-lean2.js` 를 fz-plan 기본 배선으로 전환** | 9콜 `plan-collaborative.js`(존치 — 롤백 경로) | 3,492s → **1,344s (-61.5%)** | 행 3 참조(major 3) — **노이즈 바닥(행 C, 5) 이하** | 요구 3축 closed · command 4→5 | — | — | ⛔ **임계를 '손실 0' → '노이즈 바닥 이하' 로 재등록 (2026-09-12 사용자 결정)**. 사후 조정이 아니라 **행 C 가 원 임계의 도달 불가능성을 실측**했기 때문이며, 재등록 주체는 사용자다(내가 갈아끼우지 않았다). ⛔ 근거 전부 **N=1**(노이즈 1회·채점자 1명·트리당 1회) — **실사용 3회 관찰 후 재평가**, 미달 시 scriptPath 한 줄 원복 |

> ⭐ **시간을 정하는 것은 과제가 아니라 구조다** (2026-09-11 4-run 교차): 같은 9콜 구조에서 과제를 *감사형*↔*신규구현* 으로 바꾸면 3,621s vs 3,492s (**3.6% 차**). 같은 트리에서 구조를 9콜→3콜로 바꾸면 3,492s→1,203s (**-65.5%**). D1 3콜은 두 트리에서 1,203s·1,191s (**1.0% 차**). ⛔ 따라서 wall 은 과제 난이도가 아니라 **직렬 단계 수 × 단계당 생성 시간**이 정한다.
> ⛔ **채점 계약 변경 이력**: 1차 pre-fix 채점(`wf_2d5c2b20-366`)이 StructuredOutput 5회 거부로 실패 — 출력 비대로 인자에 XML 태그가 섞였다. schema 에 `maxItems:15`(1차 실측 8건의 2배) + 길이 상한 추가 후 재실행. 행 1(1콜 대조)은 **구 계약**, 행 2(D1 대조)는 **신 계약** — 두 행의 발견 건수를 직접 비교하지 않는다.

### ⑤ direction verdict fable vs opus (신설 2026-07-06)

> 배선 근거: `plan-collaborative.js:220` stage0-direction · `:233` stage0-final을 fable로 승격(판정문 출력 — 생산 스테이지 대비 출력량 작아 비용 할증 최소). rebuttal·draft·integrate 등 생산 스테이지는 opus 유지.
