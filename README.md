...
**설명이 "바뀐 부분" 만 다루던 것은 지시받은 대로였다.** `fz-pr-digest` 해설 원칙에
"diff에 포함된 변경만 해설한다" 가 명시돼 있었다 — 원인은 모델이 아니라 규칙이었다.

축을 바꿨다. diff 를 출력의 축으로 두는 한 변경되지 않은 협력자(프로토콜·주입 지점·호출자)는
**구조적으로 등장할 수 없고**, 처음 보는 사람이 막히는 지점이 정확히 거기다. 이제 기능을 축으로
두고 폐포 5축으로 범위를 끊는다.

설명 규칙을 **공용 모듈**로 세우고 PR 없이 도는 **`fz-explain`** 을 신설했다. 게이트는 여덟이며
각각에 근거 등급을 붙였다 — ⛔ "이 게이트가 잡았을 구체적 실패를 댈 수 있는가" 를 못 대면
`[추론]` 으로 낮춘다.

⭐ Codex 교차검증이 자체 검증 6종을 통과한 것들을 잡았다 — 표 vs 산문 모순, `또는` 한 글자,
**negative fixture 가 자기 게이트를 위반**하던 것.

→ [릴리즈 노트](docs/releases/v4.27.0.md)
