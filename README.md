...
**`codex` 라는 이름이 두 가지를 가리키고 있었다.** 벤더 CLI 실행 파일이면서 동시에 fz 스킬
이름이었고, 같은 토큰이라 40개 파일에서 뒤섞여 있었다. 전역 치환을 돌리면 `codex exec` 348 곳이
깨진다. 치환 대상은 전부 하이픈·언더스코어 복합어이고 보존 대상은 전부 공백이나 슬래시로
분리돼 있어, **경계 문자로 갈라** 규칙 아홉 개로 나눴다. 67 파일 · 357 회.

검증은 개수로 했다. 보존 대상 일곱 종의 개수가 치환 전후로 같은지 대조했다 — `codex exec` 108 ·
`Codex CLI` 53 · `~/.codex/` 26 · `.codex/skills` 20 · `codex mcp` 7 · `Bash(codex *)` 3 ·
`codex --version` 2. 치환 로그만으로는 **무엇이 안 깨졌는지** 알 수 없다.

⭐ 값이 더 큰 것은 **검사기의 대상 목록이 따로 있었다**는 발견이다. `.githooks/pre-commit` 의
`SCOPED_RE`, `CLAUDE.md` In-scope 목록, lint 두 곳의 디렉토리 튜플 — 개명 후 고치지 않으면 새
디렉토리가 검사에서 조용히 빠진다. 코드 참조는 깨지면 소리가 나지만, 검사기 목록은 **검사가
정상적으로 돌면서 대상만 비어 있다**.

고아 서브커맨드 `commit`·`config` 를 없앴다(정의만 있고 호출 배선 0건). `argument-hint` 를
6 → 10 개로 넓혔다 — `verify-gates`·`drift`·`plan`·`micro-eval` 은 배선이 있는데 선언에
없었다. `--effort` 화이트리스트를 세웠다 — CLI 는 무효 effort 를 로컬에서 거부하지 않고 서버까지
왕복한 뒤 실패한다. `setup-gpt-skills.sh` 에 prune 을 넣었다 — 링크 생성 루프가 소스를
순회하므로 이름이 바뀐 스킬의 옛 심볼릭은 영원히 남았다.

⭐ GPT-6 Astra 대응에서 **문서가 아니라 캐시가 답을 줬다.** `ultra` effort 는 CLI 릴리즈 노트에만
있고 모델 페이지에 없어 "확인 실패" 로 분류됐는데, `models_cache.json` 에는 실재했다. 유효한 값을
거부하는 가드를 만들 뻔했다. 벤더 문서보다 **그 벤더 CLI 가 서버에서 받아 저장한 목록**이 최신이다.

→ [릴리즈 노트](docs/releases/v4.30.0.md)
