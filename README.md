...
**정본이 자기 Lead 모델을 한 줄도 모르고 있었다.** 가이드 9개 중 Fable 5.1 언급이 0건이었고, 정본
`fable-model-guide.md` 는 Fable 5 기준(캐시 읽기 $1 · 폴백 Opus 4.8 · "fable 1 ≈ opus 2 비용 등가")으로
서 있었다. Fable 5.1 캐시 읽기는 $0.25 로 Opus 5 의 **절반**이라 같은 트래픽 환산 비용은 2배가 아니라
**+9%** 다. `model-guide.md` 로 개명·재작성하면서 원칙("판단은 Fable, 생산은 Opus")과 동시 상한 수치는
그대로 두고 **근거만** 바꿨다 — 실제 차이는 호출당 시간 +49% 와 출력·캐시쓰기 단가 2배다.

⭐ 그 stale 을 감시해야 할 lint 가 **두 번 연속 초록**이었다. SSOT 가 `Opus 5 only` 라 Fable 5.1 을
stale 로 볼 수 없었고, SSOT 를 2-역할로 넓히자 정규식 문자 클래스가 `(`·`)`·`·` 를 못 먹어 매치가
통째로 실패했다 — 검사 0건이 위반 0건과 같은 색이었다. 뮤테이션(옛 정규식 `None` / 새 정규식 두 모델)
으로 판별했다.

**하네스는 자기 무게를 재는 눈이 없었다.** "하네스가 해롭다"는 외부 근거는 확실하고 fz 가 그 조건
(규칙 수·`⛔` 포화·스킬 로드 6주 +19~89%)을 갖고 있지만, 성능이 실제로 떨어졌다는 근거는 **0건**이었다.
새 로거 없이 — 트랜스크립트·워크플로 journal·fz-findings 가 이미 기록한다 — 집계기·릴리즈 스냅샷·
컨텍스트 0바이트 Stop 훅을 넣고 판정 기준표를 데이터보다 **먼저** 박았다. 첫 리포트: peer-review 16회에서
counter 스테이지 판정 23건 중 뒤집음 4(17%), 교차 226건 중 1(0.4%) — N≥20 이면 제거 A/B 후보를
데이터로 판정한다.

⭐ 이종 검증이 **fixture 결함 3건 + 내 판정 오류 3건**을 잡았다. self-test 25/25 가 통과하는 훅이
실데이터에서는 5필드 전부 0 이었다(fixture 가 assistant 에 `promptId` 를 넣어 뒀는데 실데이터엔 없다).
health-check FAIL 을 "기존 상태"로 판정한 것은 내 스크립트가 원인이었다. 자기 도구 출력을 그대로 믿은
지점마다 틀렸다.
