...
3. **구현 마찰 감지** (Implementation Friction Detection):
   ...
   | 값-매핑 전량 복사 *[candidate: 1 session evidence]* | canonical/형제 파일의 값 매핑(코드접두→타입 등)을 통째 복사 — 각 항목이 현 도메인에서 실제 발생하는지·소스끼리 일치하는지 미검증 (미확정 값을 사실화) | **Candidate 마찰 신호** (memory-guide Lesson Intake — 5 sessions 관측 후 활성 결정). 트리거=코드 판정(복사 감지)/해소=도메인 지식(값 유효성) 하이브리드 — 계약/도메인 확인은 Track3 pilot(apidog) 연계. 검증: 각 매핑 항목 발생 여부 확인, **정해지지 않은 값은 default(nil/else)로 확장점만**. 참조: `modules/promotion-ledger.md` L-7 + `feedback_template_authority_bias.md` 4th vector |
