...
|-------|------|--------------------------|------|
| 갱신 대상 가이드 존재 + 외부 자료 접근(WebSearch/GPT) 가능 | `/fz-modernize "fz 가이드 최신화"` (full) | Phase 0→6 순차 진행, Gate 0~6 전 체크리스트 충족 + Phase 6 AC8 broken link 0건 → Gate 6 통과 | normal |
...
| 사용자 요청에 "그냥/가볍게" 신호 포함 | `/fz-modernize light "그냥 가볍게 최신화"` | Phase 1+2 + GPT micro-eval만 실행, Phase 3~6 미진입, GPT 카운터 1 소비 | edge-case |
...
| Plan v1/v2/v3 모두 GPT needs_revision (누적 3/3) | Phase 4 Verify 4회차 진입 시점 | GPT 4회차 미실행 + 사용자 에스컬레이션 발생 → Gate 4 '누적 카운터 ≤3' 충족 | failure |
