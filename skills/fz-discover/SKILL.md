...
> 반성 교훈: /fz 없이 직접 호출 시 ASD 폴더가 초기화되지 않아 5라운드 아티팩트 전부 누락됨.
...
2. 패턴 있으면:
   - `TVING/ASD-xxxx/` 폴더 존재 확인 → 없으면 `mkdir -p` + index.md 생성
   - `TVING/ASD-xxxx/discover/` 서브폴더 생성
   - WORK_DIR 설정
3. 패턴 없으면: 비ASD 모드 (Serena Memory fallback)

### Gate 0: ASD Ready
- [ ] ⛔ 인자에서 ASD 패턴 체크 완료?
- [ ] ⛔ ASD 패턴 있으면 폴더 + index.md 생성 완료?
- [ ] WORK_DIR 결정됨?
...
6. **⛔ 아티팩트 기록** (항상 — compact recovery 필수):
   ...
   - ASD 활성: `{WORK_DIR}/discover/round-{N}.md` + `{WORK_DIR}/index.md` 업데이트
   - 비ASD: `write_memory("fz:checkpoint:discover-r{N}", "Round {N}: 새 제약 {C}. 생존 후보: {요약}")`
...
4. **⛔ 제약 매트릭스 기록** (항상 — compact recovery 필수):
   - ASD 활성: `{WORK_DIR}/discover/constraints.md` + `{WORK_DIR}/index.md` 업데이트
   - 비ASD: `write_memory("fz:checkpoint:discover-final", "제약 {N}개. 채택: {옵션}. 정제 요구사항: {요약}")`
...
> ASD 폴더 활성 시: 다음 스킬이 `{WORK_DIR}/discover/constraints.md`를 읽어 컨텍스트를 복원한다.
