...
6. **⛔ 저널 갱신** (항상 — compact recovery 필수):
   ...
   - ASD 활성: `{WORK_DIR}/discover/discover-journal.md` 덮어쓰기 + `{WORK_DIR}/index.md` 업데이트
   ...
   - 비ASD: `write_memory("fz:checkpoint:discover-current", "제약 {N}개: {C1~CN}. 생존: {후보}. 핵심 추론: {요약}")` (매 라운드 덮어쓰기)
...
4. **⛔ 저널 최종 갱신** (항상 — compact recovery 필수):
   ...
   - ASD 활성: `{WORK_DIR}/discover/discover-journal.md` Current State 최종 갱신 + `{WORK_DIR}/index.md` 업데이트
...
> ASD 폴더 활성 시: 다음 스킬이 `{WORK_DIR}/discover/discover-journal.md`의 Current State 섹션을 읽어 컨텍스트를 복원한다.
