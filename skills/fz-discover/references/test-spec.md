...
|-------|------|------|------|
| 인자에 `[A-Z]{2,6}-\d{2,5}` 패턴 포함 + 불명확한 상태 저장 위치 질문, Serena 가용 | `/fz-discover "TVG-1234 이 상태를 어디에 저장할까?"` | Phase 0에서 `{CWD}/TVG-1234/discover/` 자동 생성으로 Gate 0(Work Dir Ready) 통과 → Gate 1·2·3 체크리스트 전부 통과 → 산출물에 경로 ≥2개(전제조건/비용/리스크 열 채워짐) + Open Questions 포함 | normal |
