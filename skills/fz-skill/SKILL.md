...
description: >-
  ...
  예: 스킬 만들어줘, 에이전트 만들어줘, 스킬 평가해줘 (비사용: 가이드·문서 갱신 →fz-modernize)
user-invocable: true
...
- 문서 작성 + 글쓰기 + 프롬프트 최적화 (스킬/에이전트 외 일반 문서 포함)
...
1. **10대 원칙 체크**: `guides/prompt-optimization.md` 직접 적용 (write 섹션 체크리스트 참조)
2. **생태계 정합성**:
...
- [ ] Boundaries 섹션 포함?
- [ ] 10대 원칙 검증 통과?
- [ ] 아티팩트 기록 완료? (티켓 폴더(WORK_DIR) 활성 시)
...
3. **Runtime Trigger Eval** (`--runtime` 옵션 또는 기본 포함):
   ...
   - PASS 기준: ≥90% 정확도 (skill-testing §2.1 + 본 스킬 Gate와 통일)
...
- [ ] 리뷰형 스킬이면 §6.4 2단계(Coverage ≥90% / Verification precision ≥80%) 실행?
...
```
BAD (eval 없이 품질 단정):
create 후 "스킬 완성" 선언 → Static Analysis·트리거 정확도 미측정.

GOOD:
create → eval → Static 8항목 + Triggering ≥90% 확인 → 미달 항목 개선 제안.
리뷰형 스킬이면 §6.4 2단계(coverage/verification)까지 실행.
```
...
## write — 문서 작성 + 글쓰기 + 프롬프트 최적화

스킬/에이전트 외 일반 문서 (CLAUDE.md, 모듈, 가이드, 일반 글쓰기 콘텐츠)를 작성/검증/최적화한다.

### 사용 시점

```bash
/fz-skill write "CLAUDE.md에 새 규칙 추가"        # 일반 문서 작성
/fz-skill write --check fz-plan                   # 기존 문서 진단 (수정 X)
/fz-skill write --optimize fz-code                # 진단 + 자동 수정
```

### 절차

1. **대상 식별**: 스킬/에이전트/CLAUDE.md/모듈/가이드/일반 문서
2. **자동화 수준**: L3 (자동) / L2 (대화형) / L1 (체크리스트만)
3. **가이드 참조**:
   - `guides/prompt-optimization.md` — 10대 원칙 (모든 문서)
   - `guides/skill-authoring.md` — 스킬 문서
   - `guides/agent-team-guide.md` — 에이전트 문서
...
| 쿼리 | 예상 | 비고 |
|------|------|------|
| "스킬 만들어줘" | trigger | create 서브커맨드 (description '예:' 핵심) |
| "에이전트 만들어줘" | trigger | new-agent 서브커맨드 (description '예:') |
| "스킬 평가해줘" | trigger | eval 서브커맨드 (description '예:') |
| "fz-plan description 최적화해줘" | trigger | optimize 서브커맨드 (description "최적화") |
| "fz-old 스킬 삭제해줘" | trigger | delete 서브커맨드 (description "삭제") |
| "가이드 문서 최신화해줘" | NOT trigger | → fz-modernize (description '비사용: 가이드·문서 갱신') |
| "스킬 목록 보여줘" | NOT trigger | → fz-manage list/deps (Will Not: 목록/의존성 조회) |
| "전체 스킬 일괄 평가해줘" | NOT trigger | → fz-manage benchmark (Will Not: 일괄 평가) |
| "이 코드 수정해줘" | NOT trigger | → fz-code / fz-fix (Will Not: 코드 수정) |

### Functional Test

| Given | When | Then | type |
|-------|------|------|------|
| 스킬 이름+목적이 명확 (L3 자동 조건 충족) | `/fz-skill create fz-test "테스트 자동화 스킬"` | Gate "Skill Created" 체크리스트 8/8 통과 + 생성 파일 경로 출력 | normal |
| 대상 스킬 SKILL.md 존재 | `/fz-skill eval fz-plan` | Static Analysis 8항목 전부 실행 + Triggering 정확도 ≥90% → Gate "Eval Complete" 통과 (비리뷰형이라 §6.4 N/A — 4항목 중 3항목 적용) | normal |
| skill-creator 플러그인 미설치 | `/fz-skill eval fz-plan --runtime` | Runtime Trigger Eval = SKIP(WARN) + 설치 안내, 정적 eval만 중단 없이 계속 | edge-case |
| 삭제 대상이 타 스킬/모듈/에이전트에서 참조됨 | `/fz-skill delete fz-old` | Grep 역참조 탐지 → 영향 보고 테이블 + 경고 + AskUserQuestion, 사용자 미확인 시 삭제 미실행 | failure |
| 존재하지 않는 스킬명 지정 | `/fz-skill eval fz-nonexistent` | "eval 대상 스킬 없음" 에러 → 스킬 목록 출력 + AskUserQuestion (에러 대응 테이블) | failure |
...
- 10대 원칙으로 문서 품질 보장 (`write` 서브커맨드 — `--check` / `--optimize`)
- 일반 문서 작성 + 글쓰기 + 프롬프트 최적화 (스킬/에이전트 외 CLAUDE.md, 모듈, 가이드 등)
- 생태계 정합성 검증
