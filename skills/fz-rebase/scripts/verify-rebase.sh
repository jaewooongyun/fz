#!/usr/bin/env bash
#
# diff-parse: hunk-state — diff 헤더 인식은 `inhdr` 로 `diff --git` ~ 첫 `@@` 구간에
#   한정한다(내용 라인 "-- foo" 오인 방지). 잔여 1곳은 머지 해결 해시(아래 ⚠️ 참조).
...
scan_manual_merges() {
  ...
  for m in $(git rev-list --merges "$1"); do
    ...
    # ⚠️ 잔여: `grep -v '^+++'` 는 헤더를 **내용으로** 배제한다. 해결 라인이 `++` 로
    #    시작하면(예: `++i`) diff 상 `+++i` 가 되어 해시에서 빠진다. 영향은 좁다 —
    #    그 라인만 다른 두 해결이 같은 해시가 될 수 있다(유실 오탐이 아니라 **미검출**).
    #    엄밀히 하려면 hunk 상태 추적이 필요하다. [미수정: fz-rebase 범위 밖]
