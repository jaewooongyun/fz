...
Custom subagent 정의 파일(`.claude/agents/*.md`) 작성 시 이 템플릿을 참고하세요.
...
기본값은 sonnet이며, 특정 팀 구성에서 Primary Worker로 동작할 때 opus로 승격됩니다 (`.claude/modules/team-registry.md` 참조).
...
전체 에이전트 목록: `.claude/modules/team-registry.md` 참조.
...
- [ ] `.claude/modules/team-registry.md`에 등록했는가?
